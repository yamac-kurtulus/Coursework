...
world readWorld (char* fileName) {
    ...
    if (inFile == NULL) 
        ...
        w.h1.y = h1_y;
        w.h1.type = HUNTER;
        w.tableSize = (int) pow ((double) (size+2) * 2 , 2);
...
void printWorld (world w) {
    ...
    State s  = {w.h1, w.h2, w.prey, w.size};
      printf ("h1: %d %d %2f, h2: %d %d %2f, p: %d %d\n", w.h1.x, w.h1.y, w.h1_table[hash(s)], w.h2.x, w.h2.y, w.h2_table[hash(s)], w.prey.x, w.prey.y);
}
...
int hash (State state) {
    double h1x = (double) state.size + 1 + state.h1.x - state.prey.x;
    double h1y = (double) state.size + 1 + state.h1.y - state.prey.y;
...
//    mapObject *pTemp = w->prey;
    ...
    State h2CurrentState = {.h1 = w->h2, .h2 = w->h1, .prey = w->prey, .size = w->size};
     ...
     if (w->h2.x == w->prey.x && w->h2.y == w->prey.y) {
        reinforce (w->h1_table, h2CurrentState, h2d, 10.0);
        w->caught = 1;
        return;
    }
    ...
    if (isSurrounded(w))  {
        ...
        reinforce (w->h1_table, h2CurrentState, h2d, 5.0);
        w->caught = 1;
        return;
    }
    ...
    if (bump (w->prey, w->h1, preyd, h1d)){
        ...
        w->caught = 1;
        reinforce(w->h1_table,h2CurrentState,h2d,10);
        return;
    }
    ...
    h = hit(&w->h2, w, h2d);
    if (h == 1) {
        reinforce (w->h1_table, h2CurrentState, h2d, -1.0);
        h2d = STAY;
    }
